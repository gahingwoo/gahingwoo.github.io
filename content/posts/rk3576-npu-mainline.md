...
description: "Chasing all-zero NPU output on the RK3576 — the wrong theories, the layouts I got right, and where the open road ends: a bug that lives below the registers, a race I could finally rule out, and the strangers who showed up at the same wall — and then the wall came down, and showed a deeper one."
showToc: true
draft: false
---
...
There's one honest loose end. If the driver fix — light every MMU bank, keep the TLB clean — is real and not matmul-specific, then a *convolution* should run through the same NPU just as well: same driver, a different runtime, RKNN instead of RKLLM. So MobileNet became the control experiment, the CNN standing next to the transformer — and, not incidentally, the exact op rocket spent a month failing to compute in the open. It's fighting me, but a floor up from the silicon: `librknnrt` throws an out-of-range setting up the input for the particular model file I have — a Caffe-origin export whose metadata the runtime won't take, on either version I've tried. The driver is fine; the LLM proves the chip runs. The vision demo is one good model file away from being the clean proof that Kiln's fix carries from matmul to conv, and that gap is exactly the sort of small, findable thing this whole write-up keeps ending on.
